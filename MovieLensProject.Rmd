...
## Problem Statement
In this project, we will be creating your own recommendation system using all the tools learned during the courses in this series. To facilitate computation a smaller 10M version of the MovieLens dataset will be used.
...
There are `r sum(unique(edx$userId))` users in the *edx* dataframe. The number of movies that were rated are `r sum(unique(edx$movieId))`. The *rating* is between `r min(edx$rating)` and `r max(edx$rating)`. 
...
as.data.frame(edx$genres[1:10]) %>% 
cSplit(as.data.frame(edx$genres[1:10]),sep="\\|")
read.table(text=edx$genres[1:10], sep ="|", colClasses = "character")
